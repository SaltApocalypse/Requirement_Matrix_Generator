...
def is_merged_cell(ws: Workbook, cell: str) -> bool:
    """
    Check whether the cell is part of a merged cell or not.

    Args:
        - ws (Workbook): The workbook containing the worksheet.
        - cell (str): The cell reference, e.g., "A1".

    Returns:
        bool: True if the cell is part of a merged cell, False otherwise.
    """
    cell = ws[cell]
...
def create_workbook(column: SheetTree, row: SheetTree, title: str) -> Workbook:
    ...
    # title
    worksheet["A1"] = title
    worksheet.merge_cells("A1:" + utils.number_to_alpha(column_length) + str(row_height))

    # ========== create columns' data ==========
    sheet_number = row_height
    for i in range(column.node_nums):
        sheet_alpha = utils.number_to_alpha(column.node_level[i], 1)
    ...
    for i in range(column_height):
        tail_sheet_alpha = utils.number_to_alpha(column_length)
        ...
        if None == worksheet[tail_sheet].value:
            for j in range(1, column_length):
                head_sheet_alpha = utils.number_to_alpha(column_length, -j)
    ...
    for i in range(1, column_length):
        head_sheet_alpha = utils.number_to_alpha(i)
    ...
    # ========== create rows' data ==========
    sheet_alpha = utils.number_to_alpha(column_length)
    for i in range(row.node_nums):
        sheet_number = row.node_level[i] + 1
        if 0 == i or row.node_level[i] <= row.node_level[i - 1]:
            sheet_alpha = utils.number_to_alpha(utils.alpha_to_number(sheet_alpha), 1)
        sheet = sheet_alpha + str(sheet_number)
        worksheet[sheet] = row.tree_data[i]
    row_length = utils.alpha_to_number(sheet_alpha) - column_length

    # merge cells
    # each row from bottom to top
    for i in range(1, row_length):
        tail_sheet_alpha = utils.number_to_alpha(column_length, i)
    ...
    for i in range(1, row_height):
        head_sheet_alpha = tail_sheet_alpha = utils.number_to_alpha(column_length, 1)
        head_sheet_number = i
        head_sheet = head_sheet_alpha + str(head_sheet_number)
        while utils.alpha_to_number(head_sheet_alpha) <= column_length + row_length:
            tail_sheet_alpha = utils.number_to_alpha(utils.alpha_to_number(tail_sheet_alpha), 1)
            tail_sheet = tail_sheet_alpha + str(head_sheet_number)
            if worksheet[tail_sheet].value != None or utils.alpha_to_number(tail_sheet_alpha) > column_length + row_length:
                tail_sheet = utils.number_to_alpha(utils.alpha_to_number(tail_sheet_alpha), -1) + str(head_sheet_number)
